...
TOPIC_KEYWORDS = {
    "gcd": ["gcd", "hcf", "euclidean", "greatest common divisor", "highest common factor", "divide", "divisible", "remainder"],
    ...
    "permcomb": ["permutation", "combination", "arrange", "select", "choose", "committee", "committees", "group", "members", "team", "ncr", "npr", "how many", "ways", "formed", "possibilities"],
...
def detect_topic(text: str) -> str:
    ...
    return best if scores[best] > 0 else "permcomb" if any(w in t for w in ["choose", "select", "committee", "how many", "group"]) else "gcd"
...
def extract_permcomb_params(text: str):
    t_low = text.lower()
    if any(w in t_low for w in ["permutation", "arrange", "order", "sequence", "line", "row", "npr", "arrangement"]):
        kind = "Permutation (nPr)"
    elif any(w in t_low for w in ["combination", "choose", "select", "committee", "group", "team", "ncr", "member", "subset", "formed"]):
...
# ============================================================
    ...
    question_text = st.text_area("Enter or edit your question", value=initial_q,
                                  placeholder=f"Type your problem for {TOPICS[topic_key]}...",
                                  height=80)
    ...
    if topic_key == "gcd":
        ...
        default_b = parsed_params.get("b", 462) if topic_key == detected else 462
        ...
        c1, c2 = st.columns(2)
        default_a = parsed_params.get("a", 1.0) if topic_key == detected else 1.0
        default_b = parsed_params.get("b", 1.7320508) if topic_key == detected else 1.7320508
        ...
        default_a = parsed_params.get("a", 1.0) if topic_key == detected else 1.0
        default_b = parsed_params.get("b", 1.0) if topic_key == detected else 1.0
        default_n = parsed_params.get("n", 4) if topic_key == detected else 4
        a = c1.number_input("Real part (a)", value=float(default_a))
        b = c2.number_input("Imaginary part (b)", value=float(default_b))
        ...
        c1, c2, c3 = st.columns(3)
        default_kind = parsed_params.get("kind", "Combination (nCr)") if topic_key == detected else "Combination (nCr)"
        ...
        default_n = parsed_params.get("n", 9) if topic_key == detected else 9
        default_r = parsed_params.get("r", 4) if topic_key == detected else 4
        ...
        c1, c2, c3 = st.columns(3)
        default_expr = parsed_params.get("expr_str", "sin(3*x)/x") if topic_key == detected else "sin(3*x)/x"
        default_var = parsed_params.get("var_str", "x") if topic_key == detected else "x"
        default_point = parsed_params.get("point_str", "0") if topic_key == detected else "0"
...
if page == "🏠 Home":
    ...
    c1.button("💡 GCD: 1071 & 462", on_click=set_nav_page, args=("🧮 Euclidean Algorithm & GCD", sample_qs["gcd"]))
    c2.button("💡 Polar: 1 + 1.732i", on_click=set_nav_page, args=("📍 Complex Numbers & Polar Form", sample_qs["complex"]))
    c3.button("💡 5C2", on_click=set_nav_page, args=("🔢 Permutations & Combinations", sample_qs["permcomb"]))
    c4.button("💡 Limit: sin(3x)/x", on_click=set_nav_page, args=("📈 Limits & Continuity", sample_qs["limits"]))
...
# ============================================================
    ...
    if topic_key == "gcd":
        ...
        c1, c2, c3 = st.columns(3)
        default_kind = parsed_params.get("kind", "Permutation (nPr)") if topic_key == detected else "Permutation (nPr)"
        kind_opts = ["Permutation (nPr)", "Combination (nCr)"]
        kind_idx = kind_opts.index(default_kind) if default_kind in kind_opts else 0
        kind = c1.selectbox("Type", kind_opts, index=kind_idx)
        default_n = parsed_params.get("n", 5) if topic_key == detected else 5
        default_r = parsed_params.get("r", 2) if topic_key == detected else 2
